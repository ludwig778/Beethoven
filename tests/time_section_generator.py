...
@mark.parametrize("time_signature,duration,first,last,length", [
    # Basic tests
    (
        "4/4",
        "W",
...
def test_time_section_generator(time_signature, duration, first, last, length):
    ...
    generator = time_section_generator(time_signature, duration)
...
def test_time_section_generator_with_limit():
    ...
    assert time_sections[0] == (
        TimeSection(1, 1),
        Duration(1)
    )
    assert time_sections[-1] == (
        TimeSection(3, 4),
        Duration(1)
    )
...
@mark.parametrize("time_signature,time_section,duration", [
