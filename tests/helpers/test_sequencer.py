...
from beethoven.helpers.sequencer import sort_generator_outputs, note_repeater, note_sequencer
from beethoven.models import Duration


@mark.parametrize("cycle,offset,cycles,expected", [
    (
        Duration(value=1),
        None,
        11,
        (Duration(value=10), None)
    ),
    (
        Duration(value=1),
...
@mark.parametrize("step,round_robin,cycles,expected", [
    (
        Duration(value=1),
        "+...+",
        1,
        (Duration(value=0), None)
    ),
    (
        Duration(value=1),
        "+...+",
