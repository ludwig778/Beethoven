...
NOTE_REGEX = r"(?P<note_name>((Do|Ré|Mi|Fa|Sol|La|Si)|(do|re|ré|mi|fa|sol|la|si)|[A-G]{1}))"
...
SEQUENCER_CHORD_PARSER = re.compile(NOTE_REGEX + ALTERATION_REGEX + OCTAVE_REGEX + CHORD_NAME_REGEX)

PROGRESSION_ENTRY_PARSER = re.compile(r"(?<=(p|P)=)(?P<progression>.*)")
NOTE_ENTRY_PARSER = re.compile(r"((?<=(n|N)=)|(?<=(note|NOTE)=))(?P<note>[A-Za-z0-9#b]+)")
SCALE_ENTRY_PARSER = re.compile(r"((?<=(sc|SC)=)|(?<=(scale|SCALE)=))(?P<scale>[a-z_]*)")
TEMPO_ENTRY_PARSER = re.compile(r"((?<=(t|T)=)|(?<=(bpm|BPM)=))(?P<tempo>\d*)")
TIME_SIGNATURE_ENTRY_PARSER = re.compile(r"((?<=(ts|TS)=)|(?<=(sig|SIG)=))(?P<time_signature>[0-9/]*)")
FLAG_ENTRY_PARSER = re.compile(r"((?<=(f|F)=)|(?<=(flag|FLAG)=))(?P<flag>[A-Za-z0-9-_|]*)")
